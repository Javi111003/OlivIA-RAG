...
async def _use_llm_for_evaluation(query: str, document: str, llm: MistralLLMProvider) -> bool:
    ...
    """
    result: ChunkLabel = await llm.ainvoke(prompt)
    return result.is_relevant

async def evaluate_query(query_embedding,
                   query: str, 
    ...
    for doc, score in doc_scores:
        chunk_text = doc
        is_relevant = await _use_llm_for_evaluation(query_embedding, chunk_text, llm)
        chunk_score.append({
            "text": chunk_text,
            "score": score,
...
async def run_experiments(queries: list[str]) -> list[Dict[str, Any]]:
    retriever = DenseRetriever()
    ...
    for query in queries:
        ...
        result = await evaluate_query(query_embedding, query, retriever, llm)
...
async def run_threshold_experiment(
    ...
    results = await run_experiments(queries)
    save_results(results, output_path)
    
if __name__ == "__main__":
    asyncio.run(run_threshold_experiment(
        queries=queries_ste,
        output_path="./src/experiments/similarity_threshold/results.json"
    ))